"""Multinode LSTM for shadowed prediction setup.
...
Same channel-flatten pattern as `MultinodeGRU`: slots × C are concatenated as
the feature dim of a single seq2one LSTM (Zhao et al. style). No graph
propagation — used as a non-graph baseline.
"""
...
class MultinodeLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, sensor_id=None) -> torch.Tensor:
        # x: (B, T_in, n_slots, C). sensor_id ignored.
